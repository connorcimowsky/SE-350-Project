...
\begin{algorithm}
\caption{Memory Initialization}
\begin{algorithmic}[1]
\Procedure {k\_memory\_init}{\null}
    \State $mem\_end \leftarrow mem\_end + \texttt{NUM_PROCS} \times \texttt{PCB_SIZE}$
    \State $mem\_end \leftarrow mem\_end + \texttt{QUEUE_SIZE}$ \Comment{Ready queue}
    \State $mem\_end \leftarrow mem\_end + \texttt{QUEUE_SIZE}$ \Comment{Blocked-on-memory queue}
    \State $mem\_end \leftarrow mem\_end + \texttt{QUEUE_SIZE}$ \Comment{Blocked-on-receive queue}
    \State $mem\_end \leftarrow mem\_end + \texttt{LIST_SIZE}$ \Comment{Memory heap}
    \For{$i \leftarrow 0$ \textbf{to} \texttt{NUM_BLOCKS}}
        \State $mem\_block \leftarrow mem\_end$
        \State \Call{push}{$mem\_block$, $mem\_heap$}
        \State $mem\_end \leftarrow mem\_end + \texttt{BLOCK_SIZE}$
    \EndFor
    \For{$i \leftarrow 0$ \textbf{to} \texttt{NUM_KCD_REG}}
        \State $kcd\_entry \leftarrow mem\_end$
        \State \Call{push}{$kcd\_entry$, $kcd\_registry$}
        \State $mem\_end \leftarrow mem\_end + \texttt{KCD_ENTRY_SIZE}$
    \EndFor
\EndProcedure
\end{algorithmic}
\end{algorithm}
