...
\Procedure {k_send_message}{$recipient$, $msg$}
    \If {$recipient < 0$ \textbf{or} $recipient >=$ \texttt{NUM_PROCS}}
        \State \Return \texttt{RTOS_ERR} \Comment{Recipient must be a valid process}
    \EndIf
    \If {$\Call{k_send_message_helper}{cur\_proc, recipient, msg} = 1$}
        \If {$recipient.priority >= cur\_proc.priority$}
