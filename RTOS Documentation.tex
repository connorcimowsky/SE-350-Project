...
\Procedure {k_release_processor}{\null}
    ...
    \If {$cur\_proc.state \not = \texttt{BLOCKED}$}
        ...
        \EndIf
    \EndIf
    \State \Call{context\_switch}{$cur\_proc$, $next\_proc$}
...
\begin{algorithmic}[1]
\Procedure {context_switch}{$prev\_proc$, $next\_proc$}
...
\Procedure {k_set_process_priority}{$proc$, $priority$}
    ...
    \If{$proc.state = \texttt{NEW}$ \textbf{or} $proc.state = \texttt{READY}$}
        \State \Call{remove}{$proc$, $ready\_queue$}
        \State $proc.priority \leftarrow priority$
        \State \Call{enqueue}{$proc$, $ready\_queue$}
    \ElsIf{$proc.state = \texttt{BLOCKED_ON_MEMORY}$}
        \State \Call{remove}{$proc$, $blocked\_on\_memory\_queue$}
        \State $proc.priority \leftarrow priority$
        \State \Call{enqueue}{$proc$, $blocked\_on\_memory\_queue$}
    \ElsIf{$proc.state = \texttt{BLOCKED_ON_RECEIVE}$}
        \State \Call{remove}{$proc$, $blocked\_on\_receive\_queue$}
        \State $proc.priority \leftarrow priority$
        \State \Call{enqueue}{$proc$, $blocked\_on\_receive\_queue$}
    \Else
        \State $proc.priority \leftarrow priority$
    \EndIf
...
\Procedure {k_send_message_helper}{$sender$, $recipient$, $msg$}
    ...
    \If {$recipient.state = \texttt{BLOCKED_ON_RECEIVE}$}
        \State \Call{remove}{$recipient$, $blocked\_on\_receive\_queue$}
