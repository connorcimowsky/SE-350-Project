...
Our operating system maintains three queues in which PCBs are stored according to their scheduling priority. The ready queue contains processes in the \texttt{NEW} and \texttt{READY} states; the blocked-on-memory queue contains processes that are waiting on a memory block and are thus in the \texttt{BLOCKED_ON_MEMORY} state; and the blocked-on-receive queue contains processes that are waiting to receive a message and are thus in the \texttt{BLOCKED_ON_RECEIVE} state.
